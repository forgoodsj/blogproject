...
function check_button(){
	...
	if(button==1){
		...
		$("#gallery-header-center-left-title").html('所有照片');
		}
	if(button==2){
		$("#filter-studio").addClass(button_class);
		$("#gallery-header-center-left-title").html('baby的照片');
		}
	if(button==3){
		$("#filter-landscape").addClass(button_class);
		$("#gallery-header-center-left-title").html('叔叔的照片');
		}
}
...
$("#filter-studio").click(function() {  $container.isotope({ filter: '.baby' }); button = 2; check_button();  });
$("#filter-landscape").click(function() {  $container.isotope({ filter: '.叔叔' }); button = 3; check_button();  });
